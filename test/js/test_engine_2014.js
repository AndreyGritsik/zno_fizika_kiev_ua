...
function view(initTitle,initNumber,initPath,initTypeView,initNumberVar, initDirection, initLang, initVarArray,initName)
{
	...
	this.buildHTML=function(){
		...
		if(this.typeView=="picture")//будет ли задание в виде картинки
		{
			...
			this.bigImg.alt=this.title+textTask+this.name;
			this.bigImg.title=this.title+textTask+this.name;
		}
		if((this.typeView=="table")||(this.typeView=="table_picture"))
		{
			...
			if(this.typeView=="table_picture")
			{
				...
				'src="'+this.path+"task"+this.number+'.gif"'+' alt="'+this.title+textTask+this.name+
				'" title="'+this.title+textTask+this.name+
				'" name="TaskImg"></a> <div class="TextViewCell"><a href="javascript:imageIncrease()">'+textIncreaseImage+'</a></div>';
			}
...
function taskLoader(curTask)
{
	...
	if(listCounter==0) 
	{selectHTML="";}
    document.getElementById("ZnoTaskNumber").innerHTML=textTaskNumber+'<!--'+textTaskNumberCounter+' --><span class="TaskNumberView" id="TaskNumber" name="TaskNumberCounter"> '+curTask.view.name+'<br/> ('+curTask.mark+textMark+')</span>'+selectHTML+'</td>';
    ...
    for (var i=0; i<testLength; i++)
    {
	   ...
	   totalMark+=1*taskArray[i].mark;
        ...
        if(taskArray[i].userAnswer!="pass")
        {
            ...
            {
                nonCorrectAnswerText+=taskArray[i].view.name+"<br/>";
            }
        }
        else
        {
            totalPass+=1;
            passAnswerText+=taskArray[i].view.name+"<br/>";
        }
